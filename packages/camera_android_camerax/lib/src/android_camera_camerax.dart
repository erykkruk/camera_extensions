...
class AndroidCameraCameraX extends CameraPlatform {
  ...
  /// The configured aspect ratio for the camera.
  CameraAspectRatio _configuredAspectRatio = CameraAspectRatio.ratioDefault;
  ...
  Future<int> createCameraWithSettings(
    ...
    // Store the configured aspect ratio.
    _configuredAspectRatio =
        mediaSettings?.aspectRatio ?? CameraAspectRatio.ratioDefault;

    // Determine ResolutionSelector and QualitySelector based on
    // resolutionPreset and aspectRatio for camera UseCases.
  ...
  /// preset and aspect ratio for camera [UseCase]s.
  ///
  /// If the specified [preset] is unavailable, the camera will fall back to the
  /// closest lower resolution available.
  ///
  /// For 1:1 aspect ratio, a custom [ResolutionFilter] is used to select
  /// square resolutions from the available camera formats.
  ResolutionSelector? _getResolutionSelectorFromPreset(
    ResolutionPreset? preset,
    CameraAspectRatio configuredAspectRatio,
  ) {
    ...
    // Handle 1:1 aspect ratio specially
    if (configuredAspectRatio == CameraAspectRatio.ratio1x1) {
      return _getResolutionSelectorForSquare(preset);
    }

    // Override aspect ratio if explicitly configured
    if (configuredAspectRatio == CameraAspectRatio.ratio16x9) {
      aspectRatio = AspectRatio.ratio16To9;
    } else if (configuredAspectRatio == CameraAspectRatio.ratio4x3) {
    ...
    switch (preset) {
      ...
      case ResolutionPreset.max:
        ...
        final AspectRatioStrategy? maxAspectRatioStrategy = aspectRatio == null
            ? null
            : AspectRatioStrategy(
        ...
        return ResolutionSelector(
          resolutionStrategy: resolutionStrategy,
          aspectRatioStrategy: maxAspectRatioStrategy,
        );
      case null:
        // If no preset is specified but aspect ratio is configured,
        // still apply the aspect ratio.
        if (aspectRatio != null) {
          return ResolutionSelector(
  ...
  /// Returns a [ResolutionSelector] configured for square (1:1) aspect ratio.
  ///
